...
def trimData(data,maxVal,minVal,pltData):
    data_trim = data[np.logical_and(data[:,13] < maxVal, data[:,13] > minVal)]
    if pltData:
        plt.figure()
        plt.scatter(range(len(data_trim)),data_trim[:,13])
        plt.grid()
    return data_trim

''' Remove NaN values '''
def delData(data):
    data_del = data[~np.isnan(data[:,13])]
    return data_del
...
#training = np.delete(training,13,1) # Uncomment for Rho - last variable is zero which causes problems
#training = np.delete(training,24,1) # Uncomment for F and Rho - last variable is zero which causes problems
...
#label = np.char.array([r'$Q_{CC}$',r'$\alpha_{CC}$',r'$\beta_{CC1}$',r'$\beta_{CC2}$',r'$\beta_{CC3}$',r'$rc^{LJmin}_{CC}$',r'$rc^{LJmax}_{CC}$',r'$\epsilon_{CC}$',r'$\sigma_{CC}$',r'$\epsilon^{T}_{CCCC}$'])
label = []
...
for i in range(ninputs):
    ...
    ax.plot(X[:,i],Y_mean[:,i],'b',linewidth=2.0)
    ax.plot(X[:,i],Y_mean[:,i]+2*np.sqrt(Y_var[:,i]),'--r')
    ax.plot(X[:,i],Y_mean[:,i]-2*np.sqrt(Y_var[:,i]),'--r')
    #ax.set_xlabel(label[i])
    ax.set_xlabel(r''.join(['$X_{',str(i+1),'}$']))
    label.append(r''.join(['$X_{',str(i+1),'}$']))
...
legd = ax_pie.legend(patches, labels, loc='center', bbox_to_anchor=(-0.13, 0.5), fontsize=18,borderaxespad=0.)
